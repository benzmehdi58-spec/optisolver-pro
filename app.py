...
# ==========================================
# 2. CUSTOM SOLVER (FROM SCRATCH)
# ==========================================

class LinearOptimizationSolver:
    """
    Unified solver for Linear Programming and Integer Programming (General & Binary).
    ...
    def _restore_feasibility(self, tableau):
        """Dual Simplex Phase to handle negative RHS."""
        while True:
    ...
    def solve_ilp_branch_and_bound(self, c, A, b, mode='max', binary_only=False):
        """
        Solves ILP. 
        If binary_only=True, restricts variables to {0, 1}.
        """
        ...
        # If Binary, add initial constraints 0 <= x_i <= 1
        # Simplex handles x >= 0 by default. We just need x <= 1.
        ...
        root_id = "Root"
        stack = [(current_A, current_b, root_id, "Start")]
        ...
        while stack:
            ...
            if node_counter == 0: node_id = root_id
            
            if parent_id != "Root" and parent_id != node_id:
            ...
            if non_int_idx == -1:
                ...
                # Logic for General Integers
                val_floor = math.floor(non_int_val)
                val_ceil = math.ceil(non_int_val)
                
                # If Binary mode, ensure we don't branch outside [0,1]
                # But since we added initial constraints 0<=x<=1, floor/ceil will inevitably be 0 and 1
...
def main():
    ...
    st.markdown("Solve LP and IP problems with **Step-by-Step Visualization**.")
    ...
    with st.sidebar:
        ...
        if solver_method == "Branch and Bound (Integer)":
            is_binary = st.checkbox("Binary Variables {0,1} Only", value=False, help="Like Knapsack Problem")
            if is_binary:
                st.info("Variables restricted to 0 or 1.")
            else:
                st.info("General Integers allowed.")
    ...
    if st.button("🚀 Solve Problem"):
        ...
        try:
            if solver_method == "Simplex (Continuous)":
                ...
                with c1:
                    if val == -float('inf'):
                        st.error("Infeasible or Unbounded.")
                    else:
                        st.success("Optimal Solution Found!")
                        st.metric("Objective Value", f"{val:.4f}")
                        st.table(pd.DataFrame({"Variable": [f"x{i+1}" for i in range(num_vars)], "Value": x_res}))
                with c2:
                    if num_vars == 2:
                ...
                # Branch and Bound (Integer or Binary)
                val, x_res, tree = solver.solve_ilp_branch_and_bound(
                    c_coeffs, A_coeffs, b_vals, mode=mode, binary_only=is_binary
                )
